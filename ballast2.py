...
версия 1.01 16.03.2017
Добавить вывод профилей и планов в автокад масштаб для линии плана 1:1000 для
профиля 1:2000 горизонтальный, 1:200 вертикальный
версия 1.02 22.03.2017
Полностью переделан шаблон, улучшена точность нахождения дельта ф, новая трасса
задается двумя массивами (x2d y2d) и потом пересчитывается в xd
версия 1.03 23.08.2017
Вернулся на старый шаблон. Переписал код согласно PEP8
"""
...
def Izg_Napr(x, y, D, E):
    ...
    # E-модуль упругости, D-внешний диаметр. 2 первые и 2 последние точки
    # не считаем возвращем в МПа
    Ksi = []
...
def MaxDY(x0, L, x, y):
    # расчет расстояния (по y) от прямой через точки y(x0) и y(x0+L) до
    # минимальной точке на кривой y(x)
    x_lin = [x0, x0 + L]
...
def pline_to_acad(acad, x, y, x0, y0, xscale, yscale):
    # рисует полилинию по точкам x y в автокаде x0 y0 - начальная точка
    # отрисовки xscale yscale - масштаб отрисовки
    # (1:1000 = 1; 1:2000 = 0.5; 1:200 = 5)
    if len(x) != len(y):
...
for filename in sys.argv[1:]:
    ...
    n_b = 0.9  # к-т надежности по нагр. 0,9 бетонные грузы, 1-чугунные грузы
    ...
    # x2d=str_to_arr_rus_float(config["Coord"]["x"])

    if (len(xd) != len(yd) or len(yd) != len(ld) or len(ld) != len(zd) or len(zd) != len(y_obv)):
        print("разная длина исходных данных: xd=%d yd=%d ld=%d zd=%d y_obv=%d"
            % (len(xd), len(yd), len(ld), len(zd), len(y_obv)))
...
#    break
    ...
    yb.append(vd)
    yb.append(vd)
    if not config["Land"].getboolean("show_water", fallback=True):
...
# находим номер последней отметки Х которая попадает под переукладку x<x0+L
    ...
    worddoc = wordapp.Documents.Open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "Балластировка и переукладка ШАБЛОН_старый.docx")) # Create new Document Object
